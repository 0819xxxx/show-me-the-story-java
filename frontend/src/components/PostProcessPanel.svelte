<script>
  ...
  const statusCls = {
    pending: 'badge-ghost', running: 'badge-outline', done: 'badge-success',
            ...
            <tbody>
              {#each roadmapLocal as item, i}
                <tr>
                  ...
                  <td><span class="badge badge-xs {item.priority === 'P0' ? 'badge-error' : item.priority === 'P1' ? 'badge-outline' : 'badge-ghost'}">{item.priority}</span></td>
                  <td>
                    {#if item.status === 'pending'}
