...
<div class="flex flex-col h-full">
  ...
  <div class="border-b border-base-content/10 px-3 py-2 flex items-center gap-2 shrink-0">
    ...
    {#if $taskRunning}
      <button class="btn btn-error btn-outline btn-xs gap-1" on:click={stopTask}>{$t('chat.session.stop')}</button>
    {/if}
    <button class="btn btn-outline btn-xs" on:click={createSession} disabled={$taskRunning}>{$t('chat.session.new')}</button>
  </div>
    ...
    <div class="border-b border-base-content/10 max-h-[200px] overflow-y-auto bg-base-200 shrink-0">
      {#each sessions as s}
        ...
        >
          ...
          <button class="btn btn-ghost btn-xs text-error/60 opacity-0 group-hover:opacity-100 transition-opacity" on:click={(e) => deleteSession(s.id, e)}>{$t('common.delete')}</button>
        </div>
      {/each}
      {#if sessions.length === 0}
