...
<div class="flex flex-col h-screen bg-base-300 text-base-content overflow-hidden">
  ...
  <header class="navbar bg-base-200 border-b border-base-300 px-6 min-h-[46px] shrink-0 gap-4">
    ...
    {/if}
    {#if hasUpdate}
      <a href={latestReleaseURL} target="_blank" rel="noopener" class="badge badge-xs badge-outline gap-0.5 no-underline">
        {$t('app.newVersion')}
      </a>
    {/if}
    {#if $currentProject}
      ...
      {#if $taskRunning}
        <span class="badge badge-sm badge-outline gap-1">
          ...
          {$t('app.aiThinking')}
          <TaskTokenBadge className="badge badge-xs badge-outline font-mono border-0" />
        </span>
      {/if}
    {/if}
      ...
      <div class="flex flex-col w-1/2 min-w-[320px] border-r border-base-300 shrink-0">
        ...
        <nav class="flex bg-base-200 border-b border-base-300 px-3 py-2 shrink-0 gap-1">
          {#each [
            ...
            <button
              class="btn btn-xs text-sm px-4 gap-1.5 {$currentPage === page ? 'btn-primary' : 'btn-ghost'}"
              on:click={() => window.location.hash = '#' + page}
            >
              {#if icon}<span class="text-xs">{icon}</span>{/if}{$t(labelKey)}
            </button>
          {/each}
        </nav>
