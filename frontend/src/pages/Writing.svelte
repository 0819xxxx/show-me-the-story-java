<script>
  ...
  $: statusMeta = {
    ...
    writing:  { label: $t('writing.status.writing'),  cls: 'badge-outline', dot: 'bg-warning animate-pulse' },
    review:   { label: $t('writing.status.review'),   cls: 'badge-info',    dot: 'bg-info' },
    accepted: { label: $t('writing.status.accepted'), cls: 'badge-ghost', dot: 'bg-success' },
  };
        ...
        <div class="card-body p-4 gap-2">
          ...
          <div class="flex flex-wrap gap-2 text-xs">
            ...
            {#if fsNearTarget.length > 0}
              <span class="badge badge-outline">{$t('writing.fs.nearTarget', { n: fsNearTarget.length })}</span>
            {/if}
          </div>
          {#if fsOverdue.length > 0}
