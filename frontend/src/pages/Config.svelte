    ...
    <div class="card-body p-4 gap-2">
      ...
      {#if !charCollapse}
        ...
        <div class="flex gap-1.5">
          ...
          {#if chars.length > 0}
            <button class="btn btn-outline btn-xs" on:click={submitCharacters} disabled={$taskRunning}>{$t('config.char.submit')}</button>
          {/if}
        </div>
      {/if}
    </div>
    ...
    <div class="card-body p-4 gap-2">
      ...
      {#if !wvCollapse}
        ...
        <div class="flex gap-1.5">
          ...
          {#if allWvs.length > 0}
            <button class="btn btn-outline btn-xs" on:click={submitWorldview} disabled={$taskRunning}>{$t('config.wv.submit')}</button>
          {/if}
        </div>
      {/if}
    </div>
            ...
            {#each rels as r}
              <div class="flex items-center gap-2 bg-base-300 rounded-lg p-2.5 group">
                <div class="flex-1 min-w-0 text-sm flex items-center gap-1.5 flex-wrap">
                  ...
                  <span class="badge badge-xs badge-ghost">{r.label}</span>
