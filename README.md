...
- **单 JAR 运行**：一个 fat JAR + 一个浏览器，无需安装数据库或其他依赖
...
需要 JDK 17+、Node.js 18+ 以及 Maven 3.9+：
...
构建产物在 `target/show-me-the-story-1.0.0.jar`。

### 开发模式

```bash
# 终端 1：启动 Spring Boot 后端（:48090）
mvn spring-boot:run

# 终端 2：启动 Vite dev server（:5173，热重载，代理 /api → :48090）
cd frontend && npm run dev
```

开发模式下访问 `http://localhost:5173`，前端请求会自动代理到后端。

### 项目结构

```
src/main/java/com/showmethestory/
├── ShowMeTheStoryApplication.java    # Spring Boot 启动类
├── config/                           # Web 配置（CORS、静态资源、SPA fallback）
├── controller/                       # REST API 控制器（13 个）
├── i18n/                             # 国际化（提示词模板、消息、上下文构建）
├── model/                            # 数据模型（51 个 POJO）
└── service/                          # 业务逻辑（17 个服务类）

frontend/src/
├── App.svelte                        # 主布局
├── app.css                           # 主题 & 全局样式
├── components/                       # 通用组件（聊天面板、弹窗等）
├── lib/                              # 工具库（API、路由、i18n、SSE、Store）
└── pages/                            # 页面（配置、大纲、写作、伏笔、关系、技能）
